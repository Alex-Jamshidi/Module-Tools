...
def ls(args):
    ...
    print_output(
        populate_output(cwd, fs_items, dir_args, file_args, flag_status), flag_status
    )
...
def get_dir_args(cwd, fs_items):
    return [p for p in fs_items if (Path(cwd) / p).is_dir()]


def get_file_args(cwd, fs_items, flag_status):
    file_args = [p for p in fs_items if not (Path(cwd) / p).is_dir()]
